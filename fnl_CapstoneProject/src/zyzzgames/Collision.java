...
/**
 * The Collision class contains the different methods to check for collision
 * between the player and the game's obstacles. The player is a square, which is
 * equivalent to a rectangle with equal width and height. Since all objects are
 * either a rectangle, a line, or a circle, 3 collision detection methods are
 * required. The rectangle (intersects()) and line (intersectsLine()) methods
 * are provided by the built-in {@link Rectangle} library, while the
 * intersectsCircle() method is implemented by calculating the nearest distance
 * between the player and the circle, then checking if the difference is less
...
public class Collision {
	...
	public static boolean checkDeathCollision(int[][] slopes, int[][] sawblades, Player p) {
		...
		for (int i = 0; i < sawblades.length; i++) {
			if (intersectsCircle(new Rectangle(p.getX(), p.getY(), p.getHitbox(), p.getHitbox()), sawblades[i][0],
					sawblades[i][1], sawblades[i][2])) {
				return true;
			}
		}
